	...
	void Start () {
        ...
        foreach (string filename in files)
        {
            ...
            for(int i = 1; i < lines.Length; i++)
            {
                ...
                {
                    angle1 = Vector3.Angle(moveDir, viewDirOnPlane);
                    angle2 = Vector3.Angle(viewDirOnPlane, viewDir);
                }
                ...
                Vector3 viewDirOnPlane2 = new Vector3(float.Parse(curLine[3]), 0.0f,
                    float.Parse(curLine[5]));
                float angle3 = Vector3.Angle(viewDirOnPlane, viewDirOnPlane2);
                float angleView2Height = Vector3.Angle(viewDirOnPlane2, viewDir2);
                float angle4 = Mathf.Abs(angle2 - angleView2Height);
